...
public class InputHandler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (AssualtRifle != null)
        {
            AssualtRifle.SetActive(false);
        }

        FreeCameraLook.Instance.focusTarget = GameMasterObject.enemies[whichTarget];
    }
    void Update()
    {
    ...
    void HandleInput()
    {
        ...
        if (Input.GetButtonDown("Primary"))
        {
            ...
            AssualtRifle.SetActive(false);
            states.handleAnim.anim.SetBool("Free Run", true);
            states.handleAnim.anim.SetInteger("WhichWeapon", -1);
        }
        if (Input.GetButtonDown("Secondary"))
        {
            ...
            states.handleAnim.anim.SetBool("Free Run", false);
            states.handleAnim.anim.SetInteger("WhichWeapon", 0);
        }
        if (Input.GetButtonDown("Tertiary"))
        {

        }
        if (Input.GetButtonDown("Find Target"))
        {
            if (GameMasterObject.enemies.Count > 0)
            {
                FreeCameraLook.Instance.whichTarget++;
                if (FreeCameraLook.Instance.whichTarget >= GameMasterObject.enemies.Count)
                {
                    FreeCameraLook.Instance.whichTarget = 0;
                }
                FreeCameraLook.Instance.focusTarget = GameMasterObject.enemies[FreeCameraLook.Instance.whichTarget];
            }
        }
    }
    void UpdateStates()
